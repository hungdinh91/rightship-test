using OrderSystem.Shared.Extensions;
...
public class Result
{
    ...
    {
        return new Result(false, errorCode ?? Common.ErrorCode.Undefined, errorMessage);
    }

    public static Result<T> FailWithMessage<T>(ErrorCode? errorCode, string? errorMessage)
    {
        return new Result<T>(false, errorCode ?? Common.ErrorCode.Undefined, errorMessage);
    }
