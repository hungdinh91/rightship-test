...
public class SubmitOrderCommandHandler : IRequestHandler<SubmitOrderCommand, Result<OrderDto>>
{
    ...
    public SubmitOrderCommandHandler(IOrderRepository orderRepository, IProductService productService)
    {
    ...
    public async Task<Result<OrderDto>> Handle(SubmitOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.FailWithMessage<OrderDto>(productsResult.ErrorCode, productsResult.ErrorMessage);
        }

        var products = productsResult.Value ?? new List<ProductDto>();
